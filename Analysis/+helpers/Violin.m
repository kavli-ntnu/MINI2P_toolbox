classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            % plot the data mean
%             meanValue = mean(data); % weijian
            meanValue = median(data);
            if length(density) > 1
            ...
            obj.MeanPlot.Color =[0.2 0.2 0.2];
...
%             obj.MedianPlot = scatter(pos, quartiles(2), [], [1 1 1],'filled'); % weijian
...
%             obj.MeanPlot.Color = color;
        ...
        function yesno = get.ShowData(obj)
            if ~isempty(obj.ScatterPlot)
                yesno = strcmp(obj.ScatterPlot.Visible, 'off');
            end
        end
