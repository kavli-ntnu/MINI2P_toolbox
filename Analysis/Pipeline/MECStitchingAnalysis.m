...
%%
[RepeatCellPair_P1_C1 RepeatCellPair_P1_C2]=find((MeanOverlap_P1<1).*(MeanOverlap_P1>0.75)==1);
[RepeatCellPair_P2_C1 RepeatCellPair_P2_C2]=find((MeanOverlap_P2<1).*(MeanOverlap_P2>0.75)==1);
%% plot the overlapmap
close all
    figure 
    x0=10;
...
%% show heatmap for all grid cells
close all
Shuffling=1000;
% CellPerPlot=50;
k=1;
...
set(gca,'color',[1 1 1]);
set(gcf,'color',[1 1 1]);
%% show autocorrelation map for all grid cells
close all
Shuffling=1000;
% CellPerPlot=50;
k=1;
...
%         subplot(20,15,k,'align')
            ...
            SelectedFrame_filtered=intersect(find(~isnan(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,4*i+10))),find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,6)==1));% filter out the frames with speed valid
            SelectedFrame_filtered=intersect(SelectedFrame_filtered,find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,5)>2.5));% filter out the frames with speed threadhold
            Event_filtered=intersect(find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,4*i+12)>0),find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,6)==1));% filter out the frames with speed valid
            Event_filtered=intersect(Event_filtered,find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,5)>2.5));% filter out the frames with speed threadhold
            
            Position=StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(Event_filtered,2:3);
            Position(:,1)=circshift(Position(:,1),Bestshift);
            Position(:,2)=circshift(Position(:,2),Bestshift);
            Event=StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(Event_filtered,4*i+12);
            Max=max(Event(:));
...
Event_filtered=intersect(find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,4*i+12)>0),find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,6)==1));% filter out the frames with speed valid
Event_filtered=intersect(Event_filtered,find(StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(:,5)>2.5));% filter out the frames with speed threadhold
AnimaPosition=StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(SelectedFrame_filtered,2:3);
EventPosition=StichingPoor{FOV_ID,1}.NAAK.NAAK{1,j}(Event_filtered,2:3);
