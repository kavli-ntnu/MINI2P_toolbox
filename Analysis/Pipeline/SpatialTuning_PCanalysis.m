...
for j=1:1:ExperimentInformation.Session
    ...
    for i=1:1:ExperimentInformation.TotalCell     
        ...
        if length(Event_filtered)>MinEventCount && ExperimentInformation.CellSNR(i)>3 && ~ismember(i,ExperimentInformation.RepeatCell)
            ...
            PositionTrain_firstHalf=NAAK{1,j}(SelectedFrame_firstHalf,1:3);
            PositionTrain_secondHalf=NAAK{1,j}(SelectedFrame_secondHalf,1:3);                    
            EventTrain=NAAK{1,j}(SelectedFrame_filtered,[1 4*i+12]);
            EventTrain_firstHalf=NAAK{1,j}(SelectedFrame_firstHalf,[1 4*i+12]);
            EventTrain_secondHalf=NAAK{1,j}(SelectedFrame_secondHalf,[1 4*i+12]);   
